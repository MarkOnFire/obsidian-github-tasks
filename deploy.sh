...
if [ ! -f "dist/main.js" ]; then
    if [ -f "package.json" ]; then
        print_warning "dist/main.js not found. Building plugin..."
        npm run build
        if [ ! -f "dist/main.js" ]; then
            print_error "Build failed - dist/main.js still not found"
            exit 1
        fi
    else
        print_error "dist/main.js not found and no package.json to build from"
        exit 1
    fi
fi
...
deploy_to_vault() {
    ...
    # Deploy docs/readme if needed (optional)
